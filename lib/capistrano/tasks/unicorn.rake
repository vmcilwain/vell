set :unicorn_conf_file, "#{fetch(:running_dir)}/deploy/unicorn.conf"
set :unicorn_init_file, "#{fetch(:running_dir)}/deploy/unicorn_init.sh"
  ...
  task :generate_unicorn_conf do
     on roles(:app) do
      ...
      open(fetch(:unicorn_conf_file), 'w') do |f|
        f.puts(ERB.new(File.read(fetch(:running_dir) + "/deploy/templates/unicorn.conf.erb")).result(binding))
      end
    end
  end
  ...
  task :generate_unicorn_init do
     ...
     info "creating #{fetch(:application)} unicorn_init.sh file"
      open(fetch(:unicorn_init_file), 'w') do |f|
        f.puts(ERB.new(File.read(fetch(:running_dir) + "/deploy/templates/unicorn_init.sh.erb")).result(binding))
      end
    end
  end
